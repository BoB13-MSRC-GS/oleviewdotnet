...
namespace NtApiDotNet.Net.Dns
{
    ...
    internal class DnsPacket
    {
        ...
        private static DnsResourceRecordBase ReadResourceRecord(DnsQueryType type, byte[] data, byte[] rdata)
        {
            switch (type)
            {
                case DnsQueryType.CNAME:
                    return new DnsResourceRecordCNAME(data, rdata);
                case DnsQueryType.A:
                    return new DnsResourceRecordA(rdata);
                case DnsQueryType.AAAA:
                    return new DnsResourceRecordAAAA(rdata);
                case DnsQueryType.PTR:
                    return new DnsResourceRecordPTR(data, rdata);
                case DnsQueryType.SRV:
                    return new DnsResourceRecordSRV(data, rdata);
                default:
                    return new DnsResourceRecordUnknown()
                    {
                        RData = rdata
                    };
            }
        ...
        public byte[] ToArray()
        {
            ...
            Dictionary<string, int> string_cache = new Dictionary<string, int>();
