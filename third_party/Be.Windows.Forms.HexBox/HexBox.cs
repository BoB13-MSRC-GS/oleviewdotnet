...
namespace Be.Windows.Forms
{
	...
	public class HexBox : Control
	{
        ...
        /// <summary>
        /// List of hex annotations
        /// </summary>
        List<HexAnnotation> _annotations;
		...
		void CreateCaret()
		{
			...
			_caretHandler.CreateCaret(this, caretWidth, caretHeight);

			UpdateCaret();

			_caretHandler.ShowCaret(this);
		...
		void UpdateCaret()
		{
			...
			_caretHandler.SetCaretPos((int)p.X, (int)p.Y);
		}
		...
		void PaintHexAndStringView(Graphics g, long startByte, long endByte)
		{
			...
			Brush selBrush = new SolidBrush(_selectionForeColor);
			Brush selBrushBack = new SolidBrush(_selectionBackColor);
            HexAnnotation ann = null;
            Brush annBrush = null;
            Brush annBrushBack = null;
			...
			for(long i = startByte; i < intern_endByte+1; i++)
			{
                ...
                if (ann != null)
                {
                    isAnnotation = ann.InAnnotation(i);
                    if (!isAnnotation)
                    {
                        ann = null;
                        annBrush.Dispose();
                        annBrush = null;
                        annBrushBack.Dispose();
                        annBrushBack = null;
                    }
                }

                if (!isAnnotation && ann == null)
                {
                    ann = FindAnnotation(i);
                    if (ann != null)
                    {
                        annBrush = new SolidBrush(ann.ForeColor);
                        annBrushBack = new SolidBrush(ann.BackColor);
                        isAnnotation = true;
                    }
                }
				...
				else
				{
                    if (isAnnotation)
                    {
                        PaintHexStringSelected(g, b, annBrush, annBrushBack, gridPoint);
                    }
                    else
                    {
                        PaintHexString(g, b, brush, gridPoint);
				...
				{
					g.FillRectangle(selBrushBack, byteStringPointF.X, byteStringPointF.Y,
				...
				else
				{
                    if (isAnnotation)
                    {
                        g.FillRectangle(annBrushBack, byteStringPointF.X, byteStringPointF.Y, 
                            _charSize.Width, _charSize.Height);
                    }
        ...
        {
            HexAnnotation ann = new HexAnnotation();
            ann.StartPosition = startPos;
            ann.EndPosition = endPos;
            ann.ForeColor = foreColor;
        ...
        private HexAnnotation FindAnnotation(long pos)
        {
            foreach (HexAnnotation ann in _annotations)
            {
                if(ann.InAnnotation(pos))
                {
                    return ann;
                }
            }
        ...
        {
            int i = 0;
            while(i < _annotations.Count)
            {
                if (_annotations[i].EndPosition < pos)
                {
                    break;
                }

                if (_annotations[i].InAnnotation(pos))
                {
